...
module Index =
    ...
    let read fromIndex writeIndex (path: string) (indexPath: string) =
        ...
        if writeIndex then
            use indexStream =
                new FileStream(indexPath, FileMode.Create, FileAccess.Write, FileShare.None, 4_096, false)

            let segmentedIndex =
                readSegments fromIndex leadInBuffer stream indexStream

            ArrayPool<byte>.Shared.Return(leadInBuffer, false)
            segmentedIndex
        else
            let segmentedIndex =
                readSegments fromIndex leadInBuffer stream null
    ...
    let readAsync fromIndex writeIndex (path: string) (indexPath: string) =
        if writeIndex then
            task {
                use stream =
                    new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read, 4_096, true)

                use indexStream =
                    new FileStream(indexPath, FileMode.Create, FileAccess.Write, FileShare.None, 4_096, true)
