...
module Buffer =
  ...
  let readFloat80 (buffer: byte ReadOnlySpan byref) bigEndian =
     if bigEndian
     then { RawSignificand = readUInt64 &buffer bigEndian; RawSignExponent = readUInt16 &buffer bigEndian }
     else { RawSignExponent = readUInt16 &buffer bigEndian; RawSignificand = readUInt64 &buffer bigEndian }
  ...
  let readType (buffer: byte ReadOnlySpan byref) bigEndian =
    ...
    |> function 
       ...
       | 7u -> typeof<uint32>
       | 8u -> typeof<uint64>
       | 9u | 0x19u -> typeof<float32>
       | 10u | 0x1Au -> typeof<float>
       | 0x08000Cu -> typeof<struct (float32 * float32)>
       | 0x10000Du -> typeof<Complex>
       | 11u | 0x1Bu -> typeof<float80>
       | 0x20u -> typeof<string>
